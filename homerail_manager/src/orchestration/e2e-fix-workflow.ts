    ...
    spec: {
      ...
      agents: {
        ...
        ...Object.fromEntries(["a", "b", "c"].map(id => [`reviewer_${id}`, {
          system: "Independently review this candidate and its test evidence, including repair_context.plan, previous failure/strategy, and the parent-to-candidate round_diff. Assess whether this round addresses the retained repair target; unrelated cleanup is not a repair of a previous test or CI failure. Return Review via handoff. findings contains actionable unresolved defects only; put positive observations and review coverage in summary. An approve vote requires findings: []; use request_changes for defects and abstain for insufficient evidence, including omitted source needed to assess causality. Do not modify files, invent execution evidence, or consult another reviewer vote.",
        }])),
        judger: { system: "You are the Codex Judger. Evaluate supplied authoritative evidence, distinguish code failure from infrastructure/unknown, and return accept/revise/pause with reasons. For candidate judgment, assess repair_context.plan, previous failure/strategy, and round_diff before accepting a revision. Unrelated cleanup or a later green CI does not establish repair of the earlier failure; pause if evidence is insufficient. For CI judgment use the supplied candidate judgment, reports, and same-head CI; this stage does not provide source for another code review. An accept proposal is still checked by trusted program policy. Do not change policy or publish directly." },
      },
      nodes, edges,
    },
  };
