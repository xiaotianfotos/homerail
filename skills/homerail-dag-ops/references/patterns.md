...
Give each node a focused responsibility and explicit input/output contracts.
Use data edges when a consumer needs another node's result; use parallelism for
independent checks. Fan-in judges should rely on upstream evidence and expose
missing hard-gate evidence instead of silently repeating or inventing checks.
Keep smoke tests of wiring separate from verification of the actual work product.
...
5. Use `get_run_status` for a requested snapshot; do not repeatedly poll unchanged progress. Never claim the DAG started without the
...
per-panel correction, or later follow-up, read [multi-actor-surfaces.md](multi-actor-surfaces.md) and select a
