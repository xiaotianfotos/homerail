...
export function buildManagerAgentSystemPrompt(input: ManagerAgentPromptInput = {}): string {
  ...
  const lines = [
    ...
    "Use send_dag_actor_command only against the current waiting round returned by get_dag_supervision. Submit one atomic commands array per round; even one Actor is a one-item array. Use focus_dag_actor to draw attention to an Actor's Manager-owned Surface. Cancel or explicitly complete a waiting run only through cancel_dag_run or complete_dag_run.",
