...
export function buildManagerAgentSystemPrompt(input: ManagerAgentPromptInput = {}): string {
  ...
  const lines = [
    ...
    "Use send_dag_actor_command only against the current waiting round returned by get_dag_supervision. Use focus_dag_actor to draw attention to an Actor's Manager-owned Surface. Cancel or explicitly complete a waiting run only through cancel_dag_run or complete_dag_run.",
    "Treat milestone_digest and round_summary as Manager-owned evidence. Do not invent Worker progress, repeat raw tool logs, or narrate high-frequency progress events. A cross-Actor conclusion must cite accepted_results from the current round_summary; if required Actors are absent, say the round is incomplete.",
