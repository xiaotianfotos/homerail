# E2E Fix：统一问题清单

唯一汇总 issue：[#289](https://github.com/xiaotianfotos/homerail/issues/289)。
实施分支：`codex/e2e-fix-289`；相关实现和修复集中交付一个 PR。
完整目标、阶段和验收矩阵见 [实施计划](e2e-fix-289.md)。

## 跟踪规则（2026-09-09）

按维护者要求，本轮 AutoFix / E2E 排查发现的问题在本文件记录，不再逐项创建
GitHub issue。下面 16 个旧 issue 统一归入 #289 并关闭；这是跟踪方式调整，
**不代表每项已修复或验收通过**。旧 issue 的正文、讨论及证据链接仍保留，
原始正文也已归档到实验目录 `issue-consolidation/<number>.json`。
新增发现使用本地编号，记录复现、原因、修复与验证状态；在同一个实现 PR 中解决。
已有外部真实问题 #245 保持独立，冻结演练输入不因跟踪 issue 关闭而改变。

## 归并清单

“分支已修正”只描述当前实现，不表示已经合入主分支或完成真实 E2E 验收。

| 原 issue | 问题 | 当前证据状态 | 保留的验收要求 / 待办 |
| --- | --- | --- | --- |
| [#269](https://github.com/xiaotianfotos/homerail/issues/269) | 可信测试、持久续跑与发布对账 | 已实现部分原生阶段；总体验收未完成 | 继续覆盖 intent/执行/ack、并发与旧租约、篡改、GitHub 应答丢失；保留最佳候选、预算及全部历史。 |
| [#271](https://github.com/xiaotianfotos/homerail/issues/271) | 供应商请求归属与 token 完整性 | 仍需核对完整验收范围 | 请求关联 run/dispatch/turn；记录首 token、取消和最终 usage；区分 unknown/partial/final，验证终态后无多余推理、迟到与重复累计量不重计缓存输入。 |
| [#273](https://github.com/xiaotianfotos/homerail/issues/273) | 审查模型来源与多样性政策 | 仍需核对完整验收范围 | 报告程序解析的实际 provider/model/backend；区分独立采样与不同模型；按冻结政策验证别名、重复设置、设置变化及降级审查。 |
| [#290](https://github.com/xiaotianfotos/homerail/issues/290) | 快照文件权限受继承 ACL 影响 | 分支已修正并有回归 | 发布前归一化并核对权限；不能仅凭文件创建 mode 判断真实权限。 |
| [#291](https://github.com/xiaotianfotos/homerail/issues/291) | Docker 镜像精确 ID / RepoDigest 选择 | PR #304 已通过原生 E2E 独立验收；未合并 | 保留兼容行为及九项回归；修复正确性不能代替完整审查、发布与同 head CI。 |
| [#292](https://github.com/xiaotianfotos/homerail/issues/292) | 首次 Worker 派发前配置失败浪费 Planner | 已实现预检和严格恢复边界 | 只接受证据证明尚未派发的检查点；保留原预算、截止时间、计划与完成回执；不能用于已执行的模型失败。 |
| [#293](https://github.com/xiaotianfotos/homerail/issues/293) | HTTP 测试 fixture 丢失服务引用 | 分支已修正 | 参数化 fixture 跟踪并关闭每个已启动实例，验证调度器不会在 teardown 后继续运行。 |
| [#294](https://github.com/xiaotianfotos/homerail/issues/294) | 持久命令测试先于执行截止时间超时 | 分支已修正并有回归 | 等待覆盖执行 deadline 加有界观察开销；保留 1.1 秒合法执行、非法 JSON、退出码和唯一终态通知断言。 |
| [#295](https://github.com/xiaotianfotos/homerail/issues/295) | 输出截断后重复无效 handoff 纠正 | 已实现拒绝无证据的相同重试；模型收敛仍有限 | 同时覆盖 Worker/Node 传输和纠正预算边界；下一步必须改变批准的修复策略，保留成本；不能用停止重试宣称收敛。 |
| [#296](https://github.com/xiaotianfotos/homerail/issues/296) | 宿主 Codex schema、子进程与失败生命周期 | 分支已修正并有回归 | 严格 nullable schema；finally 清理子进程；失败命令进入原生终态并触发异常观察。 |
| [#297](https://github.com/xiaotianfotos/homerail/issues/297) | 单文件多段精确修改 | 分支已实现并有原生候选测试 | 验证不重叠的精确片段、身份与范围，拒绝含糊匹配。 |
| [#298](https://github.com/xiaotianfotos/homerail/issues/298) | 审查汇总超出上下文上限 | 已实现去重、源码 diff 投影及受限汇总恢复 | 保留全部 finding/原始报告/证据引用；真正超限仍拒绝；恢复不重跑 Fixer/测试，不重置预算。 |
| [#300](https://github.com/xiaotianfotos/homerail/issues/300) | 返修策略丢失及已驳回意见重复输入 | 分支已修正并有回归 | 所有返修分支保留 retry_strategy；只移除有完整有效证据的驳回项，历史报告不变。 |
| [#301](https://github.com/xiaotianfotos/homerail/issues/301) | 可选宿主 Fixer 与迭代异常观察 | 实现并取得真实宿主候选与测试证据 | 冻结 host_codex.fixer；观察使用真实 cycle 迭代；失败与未知执行不重发；尚需真实发布验收。 |
| [#302](https://github.com/xiaotianfotos/homerail/issues/302) | 只读测试 sandbox 与 Vitest loader 不兼容 | 具体 fixture 使用 native loader 已验证；通用机制未实现 | 需设计验收 harness 预检与受控策略修复；保留已验证候选，不修改旧失败回执冒充原测试成功。 |
| [#303](https://github.com/xiaotianfotos/homerail/issues/303) | approve 同时附 findings 的无效审查 | 新工作流已通过真实 Qwen 原生纠正验证 | 最多一次契约纠正；失败不得凑票。旧工作流已接受报告的迁移仍未实现，不能套用新 schema 宣称旧报告有效。 |

## 新增本地发现

- **L017：handoff 提示早于 Manager 接受。** 真实 Qwen 契约纠正演练中，Worker 对
  首次无效交接返回“交接成功”，随后 Manager 才以 schema 错误拒绝。当前接受账本
  正确，提示可能误导。修复方案已明确：改为“交接内容已记录，等待 Manager 校验。”，
  描述 Worker 本地暂存、由 PromptRunner 随后传输的阶段，保留参数错误语义。
  两项真实模型演练结束后已应用，现有 handoff 测试 20 项通过。没有热更新原演练镜像。
- **L018：全任务 token 硬预算。** 已保存后端报告用量和未知项，但字节、轮次及
  时间上限不等于供应商 token 硬限制。需明确预留/准入与最终计费之间的边界。

## 最近验证与整体缺口

源码 `1eb3e04b95804c1fc5c9a0eefd50a2627e6b7dc9` 的完整本地 CI 已通过，
可信完成事件为 `904ebacc3a577a705cb39f3f1c9652a3882e56b95664533e59364759b7937c71`。
开始/结束 head 一致，runner 回执和原始日志摘要已复核。这不是远端 PR CI 验收。

原生单 Reviewer 故障注入 `issue291-review-contract-proof-1` 已验证：第一次输出
批准票加正面 findings 被 Manager 拒绝；原生纠正驱动第二次新进程调用，得到
合法批准票，保留正面说明。两次调用共报告输入 37425、输出 1186 token。
逻辑会话身份保留，DSH 进程/模型上下文重建；没有重复 Fixer、测试或发布。
该诊断只证明契约纠正，不能算真实 issue 修复或 PR 成功。

真实 #291 第三次宿主演练已产生通过两类可信测试的候选，但旧契约下三份审查
均存在 approve + findings 冲突，汇总还超过上下文上限。离线证据投影已从
71740 降为 33099 字节，全部 21 条 findings 保留；没有据此伪造原生恢复成功。
旧检查点修复/迁移须单独验证，不能重置原截止时间或重复已有模型结果。

整体目标仍未完成：两个不同真实问题的端到端交付、真实负反馈修正、创建并更新
同一 PR、最终同 head 必需 CI 全绿及至少两票独立完整批准，均须按实施计划
取得可审计证据。无效审查、人工辅助恢复和模拟 GitHub 结果不能计为这些成功。

## 输出额度调整（维护者要求）

后续本地 Qwen 执行至少预留 **65536 输出 token**。原 8192 限额来自实验
Worker 镜像环境变量；旧实验结果只证明该额度下发生截断，不能据此判定模型
无法完成复杂修复。历史任务、失败及成本保持原样，新的实验使用新的冻结配置。

已配置新的实验镜像并切换隔离 Manager，Node 保留、健康检查通过。实例化的
Worker 适配器读取到 65536；服务端接受 `max_completion_tokens: 65536` 的
简短真实请求（HTTP 200，报告输入 53、输出 41 token）。服务声明上下文
262144 token，当前适配器上下文描述为 200000，后续需在该边界内预留输入、
历史工具记录与输出。输出 token 额度和 artifact 字节上限分别核对，避免转而
被序列化大小或执行超时截断。该准备检查不证明已生成 64K 内容或修复已经收敛。

### 64K 真实修复演练已启动

根任务 `issue291-e2e-qwen64-4` 使用 Qwen Fixer/Reviewer 与 Codex Planner/Judger，
沿用原问题及两项可信测试。此前 Qwen 的已验证源码补丁作为输入保留；新模型
需要完成仓库回归测试，旧候选/测试/审查不能冒充新执行证据。原生 Planner 已完成，
Fixer 已派发；实际 Worker 镜像和适配器事件均确认输出额度为 65536。
新工作流使用已验证的审查契约纠正与上下文投影。程序监督仅在异常/完成时通知。
目前仅证明新额度进入真实执行，尚未取得本次候选、审查或 PR/CI 成功证据。

### 64K 首轮候选与可信测试证据

首轮 Qwen 完成三处修改：两处既有源码精确替换，以及新建约 11KB 回归测试。
后端报告输入 13548、输出 10186 token（单个 execution），输出实际超过旧 8192
额度。可信程序形成候选 `3172543977edb016ba09c6d9f526ead525251433`，独立行为
检查九项通过，候选内 Vitest 回归十项通过；已核对候选 Git 身份、计划、回执与
原始日志摘要。该结果证明此配置能够产生完整待审候选，纠正此前基于 8K 失败
过早推断模型能力的结论。修复质量、审查及最终 PR/CI 验收仍以剩余原生阶段为准。

### 原生真实发布与第二项演练

第一项 64K 演练已得到三份有效审查和原生 Codex 候选裁决，可信发布阶段创建了
[PR #304](https://github.com/xiaotianfotos/homerail/pull/304)，head 与上述候选一致。
CI 由图内阶段派发并观察，run 为 `34289031107`；最终 CI 验收仍待结果。
原始 GitHub 观察摘要、宿主裁决的原生命令及模型事件来源已独立复核。

第二项真实问题 #245 的完整仓库测试环境已预检：基线 19 项通过；旧的只改源码
候选有 18 项通过、1 项因 `initialized` 调用序列的旧断言失败。该预检不消耗
模型调用，证明测试环境可运行且能识别真实不一致。已将完整仓库测试加入必需
检查，启动 `issue245-e2e-qwen64-5`，Codex 方案完成，Qwen 以 65536 输出额度
执行。两项任务分别保留旧结果和成本，均不把启动或 PR 创建当作 E2E 完成。

### 首个真实原生 E2E 验收通过（#291 → PR #304）

`issue291-e2e-qwen64-4` 已自行完成单个根 DAG：Codex 方案、Qwen 修复、
可信测试、三个独立 Qwen 审查会话、Codex 裁决、真实发布及最终 CI。
独立只读核验重建验收门禁，核对全部原生命令、会话派发、测试回执、Git 候选、
原始 GitHub checkout 日志及当前远端 head。候选仍为
`3172543977edb016ba09c6d9f526ead525251433`，PR #304，CI run `34289031107`
的 Linux 20/24、Windows 24、UI coverage、Docker smoke 五项必需检查均成功，
三票有效批准。原生完成事件 `98b3190baecaccff2c424800add7a0f8db192dd5e061226446d90553516d9c85`
与独立审计持久保存；未重跑模型、测试或发布，未合并 PR。

这是预先提供修复方向与既有源码补丁输入后的首轮成功，三名 Reviewer 使用相同
Qwen 模型、独立会话；不代表模型多样性，也不证明任意问题都会收敛。
本轮没有候选返修或 CI 失败，真实同 PR 的 CI 失败→修改→再验收仍待证明。
第二项 `issue245-e2e-qwen64-5` 已由 Qwen 产出候选
`385821985b62f40487595a955c376b03264c0feb`，独立初始化检查及完整仓库测试
24 项通过，原生发布 PR #305，正在等待 CI。Fixer 报告输出 24105 token，
未再受旧 8K 上限截断。整体 #289 目标仍未完成。

### 成本清单与新增证据缺口

新增只读 `scripts/e2e-fix-report.mjs`，使用方式见
[成本与尝试记录](../e2e-fix-report.md)。按执行去重累计快照，缓存输入不再次
加入总量；保留失败 Host 日志及 Worker 失败投影中的已知成本，未知用量与耗时
保留为空/不完整。根任务副本不能重复计数，报告不宣称账单完整或 E2E 验收通过。

已读取九个不同历史根任务，保留全部报告而非只选成功样本。另一个同 root 的
初始配置目录没有执行轮次，未与后来的实际执行目录重复累计，原目录仍保留。
#291 第四次演练已保存图内调用报告输入 126833、输出 14452，总计 141285 token；
其中 Qwen Fixer/Reviewer 输入 69504、输出 13791，Host Codex 输入 57329、
输出 661。这不包含外部规划对话、诊断调用及任何尚未发现的丢失记录，不能作为
整体成本或与不同旧验收配置进行严格统计对照。脚本契约 218 项通过；增加累计
计数倒退和溢出反例后，报告专属五项再次通过。运行时源码未改，无需重复原 DAG。

- **L019：Worker 模型来源回执不完整。** 真实 Fixer/Reviewer 的 ModelEvidence
  中 `model:null`，原因是当前实现读取图中的 `agent.model`，而实际运行由数据库
  setting 解析。不能查询当前可变 setting 然后伪装历史执行身份，也不能因此宣称
  模型多样性。待补：派发时持久化不含凭证的实际 backend/provider/model 与
  session/execution 绑定，角色证据引用该快照；覆盖别名、设置变化、缺失和冲突。
  当前报告如实保留 null，既有单模型演练的配置证据另存，不重写旧角色回执。

### L019 实现与验证

实际 Manager 派发审计日志已包含解析后的 provider/model，本轮改为复用该日志，
不新建重复快照。角色证据关联 session、round、generation、物理 target、lease
及后端报告的 execution；新生产阶段在证据缺失或冲突时拒绝继续。源码未查询
当前 LLM setting，原始提示/凭证不复制到新来源对象。角色 hash 同时绑定来源
对象；审查恢复验证器兼容旧/新 hash。受限旧回执恢复重新核对全部原始字段后
保留原字节，新增来源另存 sidecar，不重跑模型或改变旧票。

已在 #291 原始持久记录上确认 Fixer 和三名 Reviewer 均为独立执行、
Manager 选择的 `qwen38-flash-next`。这证明派发选择，不证明供应商内部实际
服务的模型，也不把单模型独立采样称为模型多样性。原接受报告对应三份完整角色
artifact 的摘要也再次核对一致，未改变既有运行。

Manager 类型检查及来源/失败证据 24 项测试通过；加入恢复兼容前的三项原生
Git/Docker 场景（测试→审查返修、契约纠正、模拟 GitHub CI 返修）通过。最终
完整 CI 将覆盖全部相关恢复场景。当前真实 #245 使用旧冻结运行时继续执行，
没有热更新或重复派发。

### 最新验收与执行状态

- 第二项真实问题 #245 已完成原生 E2E，PR #305 最终 head、24 项仓库测试、
  三份有效审查、原生命令及 GitHub 五项必需 CI 独立核验通过，原回执保持不变。
  两个真实任务的图内已保存角色用量分别为 141285 和 173825 token；不包含外部
  对话、额外诊断或缺失记录，也没有足够样本支持普适效率结论。
- 模型来源修复源码 `6894610f8d212c0c4e3fe6485b258ddc103440e4` 的完整本地 CI
  已通过，事件 `2155f9d4372f174d6a40b65bb8388c562738e46eab0b2560daaec65f1961262c`
  的开始/结束 head、runner 与日志摘要一致；包含恢复兼容后的原生 Docker 场景。
- L017 的提示文字修正已应用，20 项现有 handoff 测试通过；发生在上述完整 CI
  之后，最终实现 PR 仍需当前 head 的完整 CI 和审查。
- 受控真实 GitHub 故障试验已启动为 `e2e-real-ci-fault-1` / draft PR #306。
  模型角色和首轮缺陷均为明示替身/注入，真实测试与 CI 不伪造。独立 fixture base
  禁止合入 main。离线预演两轮通过，实际首轮 CI 等待中；不能预先宣称远端闭环
  或真实模型负反馈收敛成功。详见实施计划最新记录。

### 宿主准备入口收尾

实验启动脚本长期位于私有目录，无法作为交付入口。现增加
`scripts/e2e-fix.mjs freeze-runtime|prepare` 和 [操作说明](../e2e-fix.md)，
复用真实阶段的配置校验和原生工作流构建器。准备目录在候选仓库外，禁止覆盖；
先保存冻结配置、runtime、workflow/profile，最后写摘要清单作为准备完成标记。
中断保留现场，不重置旧任务。离线 profile 校验只检查结构，实际同步仍拒绝
不可用的模型设置；准备不会访问模型、测试、GitHub 或创建 run。

13 项准备/工作流/版本测试通过，包含凭证拒绝、身份与路径边界、完整摘要、
可选宿主 Fixer、失败准备保留，以及不能用离线校验绕过实际模型设置准入。
还用保留的真实配置执行了 CLI prepare，产生独立准备目录且未启动任务。
完整 CI 单独执行。统一启动、丢失创建应答对账、真实反馈最终成功和总体交付
仍未完成；新增入口不缩减原验收范围。

### 统一启动与只读创建对账

准备入口提交 `853b6ed` 的完整本地 CI 已独立核验通过（含原生 Docker 场景），
开始/结束 head、runner 和日志摘要一致，未重跑。随后增加 start/reconcile：
按准备清单核对文件，固定 Manager、原 root、workflow revision/hash 和 profile
版本，创建前原子写 intent，只有一个并发调用者能发送创建请求。Manager 提供
创建身份能力及原始请求摘要的只读查询；旧 Manager 在花费模型前被拒绝。
丢失创建应答后只读核对摘要；重复 start 不再次同步或创建，不把同名 root 当成功。

25 项准备/启动/创建幂等/HTTP 测试通过；原生 HTTP 回归证明新增查询来自持久
创建记录，且不暴露 initialPrompt。实际 CLI 对未启动准备目录返回 not_submitted
和退出码 2，没有网络或模型调用。这些是入口与传输证据，不是新增真实问题成功。
最终 head 另跑完整 CI。intent 已写但发送是否发生不明时保持 unknown；自动重发、
准入失败修复及事件监督安装仍需补齐或明确支持边界，不能声称任意启动失败可恢复。

受控 PR #306 已完成真实首轮 CI 失败→第二轮→同 PR 更新。独立核验五份原始
checkout 日志、三项真实负数断言失败、GitHub run/attempt/head、失败文本进入
第二 Planner 上下文，以及第二提交是首提交后继。首轮 run `34293274363`，
第二提交 `bf015ed9cf34a64cb669fe43acd163b919826f9e`；第二 CI `34294533900`
尚待最终验收。模型仍是明确替身，不能计作 Qwen 的实际语义返修。

### 受控远端闭环完成，真实启动暴露的恢复缺口

PR #306 已独立验收第二轮五项必需 CI 全绿；两轮同一 PR、候选祖先关系、
原始 checkout/测试日志、19 个已消费可信命令、14 个独立替身会话均核对通过。
首轮真实失败证据完整进入第二轮，非外部手工推进。该结果只放行确定性控制流，
`production_eligible` 保持 false，不把替身批准票或程序写入的补丁算真实模型成果。

启动代码 `2f1e1ed` 完整本地 CI 通过后，准备了无旧补丁输入的 #245 演练，
原基线、两个可信测试定义、范围、CI 策略及 65536 输出额度不变。实际 start
返回 unknown；只读检查发现 Worker 因交接提示源码变化而被标为 stale，模型
配置预检通过，root 尚未创建。原创建错误详情被启动器吞掉，不能事后补造原应答。
这一缺口统一记录在此，未新建 issue、未换 root 或改候选试探重跑。

已补结构化创建/恢复响应回执和 `recover-start`：先对账，在 Manager 明示支持
请求去重且没有已执行证据时，最多两次重交原样 intent；固定 root、配置版本、
请求摘要及预算身份。名额领取后应答未落盘不自动跨过；已观测/已执行的 root
丢失时要求恢复数据，禁止重新创建。23 项启动/原生创建幂等测试通过。另已构建
真实源码更新后的实验 Worker，保留已校验依赖及 Qwen 配置，尚待接入并恢复启动。
真实模型本轮尚无调用，不能报告修复或收敛成功。

## 故障矩阵补查：测试中断与返修边界

新增原生真实 Docker 的 `interrupted-test` 场景：测试子进程实际 SIGKILL，
可信 bootstrap 留下信号日志并使用保留退出码 125；同一候选至多两个测试容器，
耗尽后原生 test_route 直接暂停，不派发第二个 Fixer、Reviewer 或 Judger。
回归已通过，测试前对“可能调用 Judger 返修”的怀疑被实际路由行为否定；
未因此修改生产门禁。完整覆盖范围和其余差距见
[验收账本](e2e-fix-289-verification.md)。

真实 #245 无旧补丁演练首版仓库测试发生两处未处理 Promise rejection，
原生 Codex 明确将第二轮限定为测试中及早安装 rejection assertion，
Qwen 新会话完成修改，通过两项可信检查与三票审查并创建 PR #307。
候选 `6279b78a5323433a13b2ecdc65cb5def68340c65` 的 CI run `34297227683`
仍在执行，不能提前算完整反馈修复成功。当前已保存阶段用量 247132 token，
其中第二轮 Fixer 输出 2743 token；最终 CI Judger 尚未计入。

## 重复失败的程序约束

新增 `e2e-fix-progress.ts`：用可信失败证据生成保守指纹，第二次连续相同失败
要求改变 Codex 方案，第三次暂停。统计忽略候选/提交/会话身份变化；仅格式
改变的计划不能再次派发 Fixer。首版采用失败内容比较，不声称识别语义等价的
改写，也不能替代轮数、时间或待实现的 token 预算。

四项单元测试及两个真实 Git/Docker 原生循环通过：三份仅注释不同的错误
候选在第三次被程序暂停；另一场景重复方案被拒绝，第三次 Fixer 从未执行。
模型角色为确定性替身，此证据只验证控制流，独立于 #245 的真实模型演练。

## 真实 CI 反馈暴露的策略丢失与无关修复

PR #307 第二轮 head `6279b78a5323433a13b2ecdc65cb5def68340c65` 的 Windows
CI run `34297227683` 失败于 CLI 的 `dag-run-template-recovery.test.ts`：
模拟请求须至少四次轮询，而测试只给 30ms 真实时间。测试现用受控时间推进，
保留真实 HTTP 对账用例；21 项相关检查通过。尚未以新提交的 Windows CI 验证。

原生 CI Judger 要求返修，但 complete 阶段丢掉 retry_strategy。第三轮 Planner
明确说没有策略且无证据表明失败与允许的模型目录代码相关，却仍提出四处
测试增强；PR #307 更新为 `dda59677e4584ba21652d519b0cb953eef200c8f`，
CI run `34298573799` 已完成，五项必需 job 均成功，同 head 终态审计通过。
不能把无关修改算作修复
Windows 失败。真实第一轮未处理 Promise rejection → 第二轮修复是另一条
有效候选反馈证据，须分别评价。所有旧模型输出和策略保持不变。

CI feedback 已保留 Judger 策略；Plan 提供可选 blocked_reason，宿主严格
schema 使用 nullable 传输。Planner 明确报告无法制定方案时，程序留存 blocker
并停止，不扩大范围。根据后续用户决定，blocker 不再是关联性判断的强制要求；
相关性由 Reviewer 多票判断。宿主 schema、原生停止和策略传递回归已通过。

同时新增两个独立 Manager 进程故障试验：在原生第二轮反馈工作执行中、
以及已完成但未消费时 SIGKILL 后重启，保留第二轮会话与计数、首轮不重跑，
两次命令恰好各消费一次，owner epoch 为 1/2。模型/GitHub 不在该故障 fixture
中调用；原真实演练没有被故障注入或人工推进。

## 返修原因没有进入 Reviewer 输入

此前 Reviewer 只收到 issue、当前源码和测试结果，缺少本轮方案、上轮失败/策略
以及本轮差异。#307 第三轮无关增强仍获通过暴露了此缺口。现将这些信息从
可信冻结产物和真实 Git 父提交投影给各独立 Reviewer 及候选 Judger，更新其
语义判断要求。保留完整源码及摘要，超限时明确标注累计差异的源码缺失范围；
既不静默丢弃返修目标，也不声称所有 Reviewer 看到了被省略的源码。

五项定向检查通过：实际 Git 父提交与差异身份、投影不丢返修证据、原生 CI
反馈到三名 Reviewer 的实际派发输入、既有聚合预算与超限拒绝，以及真实
Git/Docker 流程在审查准入前投影超限源码后仍保留返修上下文。这里的模型
角色为替身，只证明数据流/边界，尚未证明新提示能消除真实模型的语义误判。

#307 独立终态审计保留三轮身份、新会话、可信执行及同 head 五项 CI 证据，
累计后端报告输入 382253、输出 49856，共 432109 token；不含外围诊断/本对话
成本，也不是账单。第一轮到第二轮是真实错误修正，第三轮不能计作 CI 因果
修复。原始证据及另存的语义审计保留，不重跑该任务。

## 审查权限调整：采用简单独立多票

真实诊断 `e2e-review-causality-proof-2` 读取 #307 原有第二/三轮证据，进行了
四次独立 Qwen 审查：两次新上下文的第三轮审查、一次第二轮有效修复审查、
一次旧输入对照。四次均 approve；前两次明确判断 Windows 失败与当前候选
无关，并据代码正确性投票。输入合计 102617、输出 4969 token，实际输出
配置均为 65536，未重复 Fixer、测试或 PR 操作。这未支持“仅增加上下文就会
使 Reviewer 否决无关返修”的原实验假设，不把结果改写为假设得到验证。

用户明确选择尊重 Reviewer 的相关性判断，采用简单多票机制。已经撤回尚未
提交的 repair_assessment/target 字段及程序拦截草稿，并移除强制因果证明的
提示。上下文继续提供作参考；Judger 核对有效独立票数、意见处置与可信测试/CI，
不因自己的关联性观点另加否决门槛。此前“第三轮不能证明修复 Windows”的
事实记录保留为实验解读，不作为拒绝有效多数票或交付的附加条件。

诊断准备曾因缺少 run-input contract 在编译时拒绝；第一次原生启动因所选
解释器不在白名单中失败，未调用模型。改用已经获准的冻结解释器后完成。
失败现场都保留，属于实验配置错误；没有为此更改产品白名单或增加 issue。


## CI 观察被瞬时错误中断，重入重置期限并冲突

三项新增回归先在旧实现失败：一次 run GET 异常立即返回 unknown；终态后
GitHub 增加元数据导致不可变证据写入冲突；观察进程中断后重入重新计算期限。
已改为保存原期限、仅重试传输读取、复用原终态证据并核对当前身份。持续
不可用仍保留 unknown；没有新增审核字段或语义门槛。

17 项 GitHub provider 测试及 Manager 类型检查通过，包括日志读取重试、
持续故障不延长期限、终态重放时 head/attempt 漂移拒绝，以及实际观察子进程
SIGKILL 后复用原 run/attempt/期限且总共一次 dispatch。该测试使用 GitHub
替身。独立真实 GitHub 只读诊断复用 #307 原第三轮已完成 CI，实际杀死 PID
3136843 后由 3136978 恢复，14 次 GET、零修改请求、零模型调用；五项必需
job 成功且 checkout head 相同。原始任务证据摘要未改变。诊断不是实际
GitHub 服务断网或原生 Manager 阶段自动重启的证明，相关差距仍保留。


## 依赖准备失败被误判为代码失败

真实 Docker 原生 DAG 回归先在旧实现复现：缺失 `workspace_template` 使
bootstrap 以 1 退出，测试结果被标成 code_failure。已将复制/权限准备异常
归为保留的基础设施退出状态 125，并增加可选、冻结的 `setup_argv`：安装
或准备失败不会执行断言，也不会立即交给 Fixer 改业务逻辑。该命令和测试
共享隔离容器及超时，网络仍关闭；真正断言失败不被吞成基础设施故障。

六个实际 Git/Docker 原生用例通过：64MiB 容器真实 OOM（两次 receipt 的
Docker OOMKilled 均为 true）、离线 npm ci 缺少 lockfile 的真实 EUSAGE、
缺失依赖快照、SIGKILL、准备成功、原三轮测试/审查返修。四种故障均保留
同一候选的两次不同容器回执，Planner/Fixer 各一次，零 Reviewer/Judger/
PR 调用，最终 cancelled。成功准备和三轮修复分别正常完成。模型与 GitHub
是明确标注的替身；不能将这些计数当成实际模型 token 使用。另 14 项隔离
契约检查与 Manager 类型检查通过，覆盖无效/篡改 setup 参数的执行前拒绝。


## 同题单次修复与预算能力核验

独立原生单次对照 `issue245-single-shot-baseline-1` 在相同 #245/base/配置/
冻结测试下首轮通过，两项真实 Docker 检查及模型/命令来源已独立核对。
Planner+Fixer 共 68079 token；原循环第一次 70304 且失败，含第一次 Judger
到第二轮过测共 150771。详情见 [成本对照](e2e-fix-289-cost-comparison.md)。
结果不支持“循环必然省 token”，支持已有循环挽救一次真实失败。

还发现性能记录的明确边界：Worker prompt-runner 收到成功 handoff 后提前
退出事件迭代，可能未消费 backend done 的 duration。此时记录的用量有效，
但没有模型耗时；本次报告保留 unknown，不把整图时间或字节计数当作生成速率。

Codex CLI 0.153.4 的独立受限通路探针，在 turn/start 前设置 tokenBudget=1，
实际收到 `ephemeral thread does not support goals`，未发出 turn/start，零
模型调用。现有宿主角色使用临时会话，不能直接借用 goal API 实现其硬预算。
[官方 app-server 文档](https://learn.chatgpt.com/docs/app-server#manage-a-thread-goal)
描述 goal 状态/预算接口，[配置参考](https://learn.chatgpt.com/docs/config-file/config-reference)
将上下文窗口和自动压缩阈值分列；这些设置不能单凭名称当作单次输出硬限制。
探针只是当前通路的能力结果，不证明其他通路都没有硬限制。全任务 token
硬预算仍未实现；已向用户提出交付范围选择，未默认视为批准延期。


## 真实流式 Worker 丢失与未计量证据

`issue245-stream-loss-proof-1` 使用原问题/模型配置和冻结运行时，运行新的
Codex 方案及 Qwen Fixer。程序确认已落盘的 4033 个流事件、16385 字节输出后，
仅对该根任务专属 Worker 执行 SIGKILL；Docker 记录退出 137。原生断连处理
把 Fixer 标记失败，capture 保留 unknown、空候选，五条持久命令各消费一次。
诊断图在 capture 后结束，没有测试、审查、发布或自动第二次模型调用。

已知 Codex 方案用量 23534 token；Worker 在返回最终 usage 前退出，其用量
未知，16385 字节不能换算为 token 或计成零成本。原始输出观察留在持久日志中，
但旧失败摘要仅遍历 usage，导致这次未计量执行从 attempts 丢失。新增回归先在
旧实现失败；修正取 usage 与输出观察的执行 ID 并集，缺失用量为 null，混合
已知和未知用量明确标记 partial。成本报表继续标记 incomplete。

冻结演练原件未改写；当前修正通过回归测试，不宣称已部署到该旧运行时。
尝试只读重投影终态证据时，现有 provenance 校验拒绝：根完成清理已将节点
会话状态改为 completed，节点状态仍为 FAILED；故保留原生 capture 时已写下的
失败证据，不篡改会话状态来绕过校验。此项实验证明故障边界，不代表丢失的模型
会话可无损接续，也不作为完整 E2E 通过。


## Manager 退出时接回原 CI 观察阶段

新增两个实际 Manager SIGKILL 用例：CI 观察仍在运行，以及观察已完成但
Manager 尚未消费回执。测试调用真实 CI provider，通过注入 GitHub transport
提供固定远端状态；Manager 重启只使用正常冷恢复启动钩子，不手写 handoff。
两种情况下观察进程 PID 均保持不变，候选、CI run/attempt、原截止时间不变，
原生命令 owner epoch 从 1 变为 2，唯一结果仅消费一次，CI 派发一次、模型调用零次。
保留注入信号、前后 Manager 身份、观察请求、原始 stdout/stderr/receipt 和独立审计。

第一版测试在 run_completed 回调内读取消费标记，看到事务尚未提交时的 0，
因此两个断言失败。核对 consumeDurableCommand 后，测试改在下一事件循环读取
已提交事实；未修改产品事务或放宽断言。随后全部 19 项 GitHub 阶段测试通过，
最终两个恢复用例再次导出并核验原始证据。这补充原生 CI 等待恢复证明，不等同
真实 GitHub 服务故障，也没有重复真实模型或创建实验 PR。


## 完整模型请求的容量与缓存用量归一化

本地 `/v1/models` 声明 262144 token，但服务没有 `/tokenize` 或 `/v1/tokenize`。
直接加载本地 tokenizer 的第一次短请求预测 69、服务报告 57；核对模板发现
必须传入实际 reasoning_effort=low，其模板计数正好为 57。服务调度器在剩余
窗口不足时会下调 max_tokens，所以仅记录请求参数 65536 不等于始终留足该额度。

新增可选文本 token gateway，在完整消息/工具/模板渲染之后、上游生成之前
检查输入加输出预留。Worker 先认证并核对策略摘要、模型与窗口再启动 DSH。
工具调用探测还发现 FreeToken 会忽略函数 strict 字段；声明的格式适配已按其
消息、工具选择和参数解码规则计数。禁止把此特定服务的规则泛化成所有服务。

第一次实际 Docker DSH 探测另暴露缓存用量漏计：pi-ai/DSH inputTokens 是未缓存
部分，cacheRead/Write 是独立类别；HomeRail 约定 input_tokens 包含这些部分。
旧测试先复现 7+2 被错误记录为 7，现改为 9；同时覆盖缓存写入。新运行记录
usage_semantics=input_tokens_include_cache，便于区分旧数据。

第二次 Docker DSH 探测经策略摘要绑定后实际完成两次本地生成、一次工具调用：
计数 338/409 分别匹配服务 usage 增量，累计 747 输入、78 输出，640 缓存输入
已包含在输入总量中。一个超容量请求在代理返回 400，未增加上游调用。
这是请求边界证明，尚不代表配置后的完整原生 root DAG 或 PR 通过。
旧探测、归一化前后证据均保留。四个主要已验收任务/单次对照的 DSH usage
报告缓存读写为零，已独立复核其对照数字不受此修正影响。

## 演练 PR 收拢与实际修复保留

维护者要求所有新验证在本地执行，不为测试创建 PR。本轮的 #304、#305、
#306、#307 已关闭且均未合并；原始 patch、运行证据与关闭回执留在本地。
#306 是受控 CI 故障 fixture，其代码不进入实现。后续唯一实现入口是 #308。
历史 PR/CI 记录只证明当时的演练结果，不代表当前交付状态或实现 PR 的审查票。

保留两项有实际用途且已有独立审查/测试证据的修复，集中整合到同一实现分支：
Worker 镜像选择支持完整 image ID/RepoDigest（#304）；Codex 模型发现等待
initialize 成功并写出 initialized 后再发送 model/list，正确处理写失败、
重复初始化响应和迟到回调（采用 #307 最终候选，替代重复的 #305）。
本地合并后 Manager 类型检查及三组相关测试共 77 项通过；完整集成 CI 和
最终实现 PR 的同 head 审查仍分别验收，不沿用历史演练票数。

整合后的首次完整本地 CI 在第二轮 feedback/SIGKILL 用例发现夹具竞态：
父进程等到 `start-manager.json` 存在即读取，子进程直接写最终路径，导致
`Unexpected end of JSON input`。保留 ca2a5ea 的失败回执与日志；将三个
Manager 恢复夹具的就绪/完成 JSON 改为同目录临时文件写完后 rename 发布。
这修复测试证据的跨进程就绪信号，不改业务恢复策略，也不放宽断言。

第二次完整 CI 通过上述 Manager 用例后，在 Worker 工作区边界测试失败：
临时工作树借用了主工作树的 node_modules 符号链接，触发路径越界拒绝。
修正临时测试环境为各包独立的本地依赖目录，保留工作区边界策略；37 项
prompt-runner 测试随后通过。第三次完整 CI 在相同代码 780b330 上通过，
耗时 408 秒，包含真实 Docker 恢复用例；两次失败日志和三次执行身份均保留。
可信执行回执、日志摘要与测试前后 HEAD 已核对，尚不能替代新 head 远端验收。

本地 `qwen-native-admission-proof-1` 补齐网关原生派发验证：一次模型执行，
完整输入 684、输出 75 token，Worker 的策略摘要/输出额度与网关逐请求证据
一致，Docker 创建、启动和清理事件绑定实际容器与镜像。图中没有发布节点。
准备时缺少 run input contract 被编译器拒绝、误用 provider_id 被 API 400
拒绝，两者均为零模型调用且已保留。相同 provider/model 设置采用 upsert，
本次更新了隔离实验环境的原有模型设置，没有更改生产环境。

## 实现 PR 审查：最后一轮纠错挤占最终 CI 判断的派发次数

PR #308 的 df0cd1c 收到两票 approve、一票 request_changes，完整报告保留一个
有效问题：原 `maxRounds * 7` 只覆盖每轮七个模型角色的正常派发，没有计入
工作流自身允许的契约纠错和瞬时派发重试。原生 Docker 用例将最多轮数设为 1，
让 review_a 修正一次不合法报告，真实测试和模拟 CI 都通过后，judge_ci 被
派发上限中止；本地先失败，证实该问题而非直接重跑审查。

派发次数按每轮七个角色，加整个 root 中每角色一次契约纠错及一次派发重试
计算；修复轮数、纠错次数和重试次数不增加。原始单轮纠错用例修复后以八次
派发完成；新增“Planner 派发暂时失败后重试 + Reviewer 契约纠错”组合，以
九次计数完成同一轮最终 CI 判断。两项真实 Docker 原生阶段测试均通过，模型
和 GitHub 传输是明确标注的替身，零真实 PR 写入。完整回归及新 head 审查另验。

维护者随后明确“不限制预算”：不增加 token 总预算或宿主调用前 token 预留
门槛。模型的实际上下文窗口仍是能力边界，不能把此决定解释成模型有无限窗口。
