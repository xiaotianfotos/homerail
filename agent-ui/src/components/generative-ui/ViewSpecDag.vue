...
import type { HomerailViewModelItemV1 } from 'homerail-protocol'

const props = defineProps<{ items: HomerailViewModelItemV1[] }>()
const nodeWidth = 154
const nodeHeight = 64
const columnGap = 52
const rowGap = 14
...
const layout = computed(() => {
  ...
  const groups = new Map<number, HomerailViewModelItemV1[]>()
...
function nodeStyle(item: HomerailViewModelItemV1): CSSProperties {
...
<template>
  <div class="hr-view__dag-scroll">
    <div class="hr-view__dag" :style="{ width: `${layout.width}px`, height: `${layout.height}px` }">
      <svg :viewBox="`0 0 ${layout.width} ${layout.height}`" aria-hidden="true">
        <defs><marker id="hr-view-arrow" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0,0 L7,3.5 L0,7 Z" /></marker></defs>
        <path v-for="edge in layout.edges" :key="edge.id" :d="edge.path" :data-tone="edge.tone" marker-end="url(#hr-view-arrow)" />
      </svg>
      <article v-for="item in items" :key="item.id" class="hr-view__dag-node" :data-tone="item.tone" :style="nodeStyle(item)">
