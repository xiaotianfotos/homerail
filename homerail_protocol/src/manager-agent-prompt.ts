...
export function buildManagerAgentSystemPrompt(input: ManagerAgentPromptInput = {}): string {
  ...
  const lines = [
    ...
    "Use send_dag_actor_command only after get_dag_supervision. For an active round, copy each target Actor's opaque state_token into that command's expected_state_token and supply a stable per-Actor idempotency_key. For a waiting round, supply the exact expected_round_id so Manager uses the existing safe multiround resume boundary. Submit one atomic commands array; even one Actor is a one-item array. Never infer transport fences or add domain-specific routing. Use focus_dag_actor to draw attention to an Actor's Manager-owned Surface. Cancel or explicitly complete a waiting run only through cancel_dag_run or complete_dag_run.",
