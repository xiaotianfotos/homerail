...
[E2E Fix](docs/e2e-fix.md) 接收既定设计，由本地 Fixer 开发并通过可信测试，
再创建或更新同一个 PR，交给独立模型审查。票数不足或 CI 失败时，
意见和真实日志自动反馈给新的 Fixer 会话继续修改。新任务不依赖 Codex，
没有 Planner/Judger 节点；配置的多票审查和同 head CI 决定是否完成。
设计、历史候选和每轮证据保留；恢复边界见[恢复说明](docs/e2e-fix-recovery.md)。
旧冻结任务的宿主 Codex 记录仍可读取，新 prepare 入口不再接受该配置。
可信候选快照会规范存储权限并通过校验后再发布；已发布但校验失败的快照会被拒绝，不会被改写。
DSH 还提供有界且不含原文的输出字节诊断，区分观察到的推理、正文和工具参数；
这不等于 token 预算限制。
程序观察器还会在宿主阶段已有失败记录却仍显示 RUNNING 时发出通知；
修复循环已接管的预期失败保持静默。
...
`--ui-public-url`（或 `HOMERAIL_UI_PUBLIC_URL`）必须是精确的 `http(s)`
Origin，不能包含通配符、路径、查询参数、片段或凭据。未显式配置时，
受保护的 UI 操作只接受 localhost、`.localhost` 或字面 IP；
`homerail.lan`、mDNS 名称和自定义域名必须通过该参数固定，否则相关请求
