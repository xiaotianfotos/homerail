...
| 每轮新上下文、完整证据与成本 | #304/#305 独立审计通过；`scripts/e2e-fix-report.mjs` 去重累计 usage，保留未知字段 | 真实返修新会话已随 #307 最终审计；可选本地 tokenizer 路由有实际双请求及原生单节点派发证明。原始回执绑定的 15 个宿主 Codex 新会话最大输入 31059 token、报告窗口 258400；额外按 65536 输出余量核对仍有至少 161805 余量。这是回顾验证，不冒称调用前预算限制；维护者明确不增加 token 预算门槛 |
...
| intent 后、进程启动前退出 | `durable-command.test.ts` intent prepared before restart；额外本地进程证明在实际 detached runner spawn 前 SIGSTOP/SIGKILL Manager | 测试、第二轮反馈、CI 三个阶段均恢复原执行身份，owner epoch 1→2、一次消费且无额外模型调用；CI 使用注入传输，零真实 GitHub 写入 |
...
| 相同失败、无效修改、超预算 | `e2e-fix-candidates.test.ts` no-op/重叠/旧片段拒绝；工作流轮数上限；截断后相同计划拒绝 | 保守失败指纹第二次重规划、第三次暂停已验证；变动诊断可能不匹配。维护者明确不增加全任务 token 预算门槛。审查发现派发计数未给已允许的纠错/重试留余量，已本地复现并修正，最终仍待同 head CI/审查 |
...
2. 保留已证明的恢复边界及未保证范围，集中修复本次审查确认的问题，不增加 token 预算门槛。
