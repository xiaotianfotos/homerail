...
import { launchE2eFix, e2eFixManagerUrl, type E2eFixLaunchTransport } from "../src/runtime/e2e-fix-launch.js";
...
describe.skipIf(process.platform !== "linux")("E2E Fix start and read-only reconciliation", () => {
  ...
  function manager(options: { loseResponse?: boolean; missingAfterCreate?: boolean; mismatch?: boolean; legacy?: boolean } = {}) {
    ...
    const transport: E2eFixLaunchTransport = async (route, body: any) => {
      ...
      if (route === "/api/e2e-fix/capabilities") return options.legacy ? null : { creation_identity_version: 1 };
